...
CLASS oXML INHERIT HObject
	...
	CLASS VAR cTypes	INIT "EAU*"	// Types of nodes allowed as prefix of name: Element(E_), Attribute(A_), User(U_), Any(*_)
...
// Any node
	...
	METHOD FindElement(cPath, lFirst, aXml)	// Find cPath in oXml tree
	METHOD FindNodes(uName, iMode, aXml)	// Find nodes with cName in oXml tree from the root; Faster than Findelements
	METHOD FromXML(sXml, cElement)		// Filling oXML tree from sXml string using mxml
	METHOD GetNameInfo( cName, lUnique )	// Get name description {Area, originalName, fullName, value}
	METHOD GetSiblings()			// Get array of all siblings with the same name
	METHOD GetSons(cName)			// Get array of all sons with the name cName, or all sons, if cName = NIL
	...
	METHOD NameFits( s, c )			// Check if search string s (wildcarded) fits cXmlName
...
//-------------------------------------------------------------------------------------------
METHOD FindElement(cPath, lFirst, aXml) CLASS oXML	// Find cPath in oXml tree; "*" $ cPath means wildcard, and function returns 
							// 	array of selected objects, rather than first object found
							// lFirst - root call
							// aXml - list of objects that fit cPath
							// Returns: Single object that fits, or array of objects, if aXml is present
							// 	in the root call, or "*" $ cPath
	
	LOCAL i, j, k, cName, r:=NIL, aFound, aNodes, cOrigPath, cNewPath, lAsArray:=.F., lUnique, cNode
	IF IsNil(lFirst);	lFirst := .T.;	ENDIF	// Root call
	IF lFirst .AND. !Empty(cPath)
		IF " " $ cPath;	cPath := Strtran(cPath, " ", "");	ENDIF
//		cPath := StrTran(cPath, ":", "\")	// ":" can be part of name, as namespace
		cPath := StrTran(cPath, "/", "\")
		cPath := StrTran(cPath, "...", "\...")
		DO WHILE "\\" $ cPath;		cPath := StrTran(cPath, "\\", "\");	ENDDO
		DO WHILE "...\" $ cPath;	cPath := StrTran(cPath, "...\", "...");	ENDDO
		IF Left(cPath,1) = "\";		cPath := Substr(cPath,2);		ENDIF
		lAsArray := !IsNil(aXml) .OR. ("*" $ StrTran(cPath, "*_", ""))			// Result as array
		IF lAsArray .AND. IsNil(aXml);	aXml := {};				ENDIF
	ENDIF
	...
	IF !Empty(cPath)
		...
		FOR k:=1 TO Len(aNodes)
			...
			cNode := aNodes[k]
			DO CASE
				CASE Empty(aFound := ::GetNameInfo(cNode, lUnique))	// cName not found
			
				CASE aFound[1] # "E"					// A_/U_ found:
					r := aFound[4]					//	Just value
					IF !Empty(r) .AND. !lUnique .AND. AScan(aXml, r) = 0;	AAdd(aXml, r);	ENDIF
					
				CASE !Empty(cNewPath)					// "E" found, non-terminal node
					FOR i:=1 TO Len(aFound[4])			//	{Elements}, Look further
						DO CASE
							CASE Empty(r := aFound[4,i]:FindElement(cNewPath, .F., aXml))
							CASE lUnique;		EXIT
							CASE AScan(aXml, r) > 0
							OTHERWISE;		AAdd(aXml, r)
						ENDCASE
					NEXT

				CASE k = 1						// "E" found, terminal node
					r := aFound[4,1]
					IF !Empty(r) .AND. !lUnique .AND. AScan(aXml, r) = 0;	AAdd(aXml, r);	ENDIF
			ENDCASE
			IF lUnique .AND. !Empty(r);	EXIT;	ENDIF
		NEXT
	ENDIF
	RETURN IIF(lAsArray, aXml, r)	// Only root call can return array

//-------------------------------------------------------------------------------------------
METHOD FindNodes(uName, iMode, aXml) CLASS oXML	// Find nodes with cName in oXml tree from the root; Faster than Findelements
						//	but no chains, ..., \ etc, only just name (wildcards)
						// iMode:	1	Get oXML (otherwise Get oXml:Values)
	LOCAL i, k, y, c
	LOCAL a:=::ParseName(uName)
	LOCAL cArea := a[1]
	LOCAL cName := a[2]

	IF IsNil(iMode);	iMode := 0;	ENDIF
	IF IsNil(aXml);		aXml := {};	ENDIF

	IF cArea $ "E*" .AND. ::NameFits(cName, ::cXmlName)
		AAdd(aXml, IIF(IsSet(iMode, 1), SELF, ::Value))
	ENDIF
	IF cArea $ "A*" .AND. !IsSet(iMode, 1)		// Attributes: Values only
		FOR EACH y IN ::hXmlAttributes
			c := y:__enumKey()
			IF ::NameFits(cName, c)
				AAdd(aXml, y:__enumValue())
			ENDIF
		NEXT
	ENDIF

	IF cArea $ "U*" .AND. !IsSet(iMode, 1)		// User vars: Values only
	...
	k := Len(::aXmlElements)
	FOR i:=1 TO k
		::aXmlElements[i]:FindNodes(uName, iMode, aXml)
	NEXT
	RETURN aXml
		...
		ELSE							// 	Find first top-level element
			IF mxmlGetType( pRoot ) = MXML_ELEMENT
				IF (cName := mxmlGetElement(pRoot)) = "?"
					...
					IF !(StrTran(::cXmlHeader," ","") == StrTran(cName," ",""))
...
//-------------------------------------------------------------------------------------------
METHOD GetNameInfo( cName, lUnique ) CLASS oXML		// Get name description {Area, originalName, fullName, value}
...
//	LOCAL cMods					// Modifiers
	...
	IF IsNil(lUnique);	lUnique := .T.;	ENDIF
	cArea := Left(a[1],1)  				// Area to search name. Sorry first char only.
		...
		IF !Empty(aFound)
			IF Len(aFound) > 1
				...
				FOR i:=2 TO Len(aFound)			// "E": If there are many elements, Area and FullName must be EQUAL
					FOR j:=1 TO i-1			// But if wildcarded, FullName can differ
						IF !(aFound[i,1] == aFound[j,1]) .OR. ;			// Area must be the same
						   (lUnique .AND. !(aFound[i,3] == aFound[j,3]))	// FullName
...
//-------------------------------------------------------------------------------------------
METHOD GetSons(cName) CLASS oXML	// Get array of all sons with the name cName, or all sons, if cName = NIL
	LOCAL i, a:={}, aEl
	aEl := ::aXmlElements
	FOR i:=1 TO Len(aEl)
		IF IsNil(cName) .OR. aEl[i]:cXmlName == cName;	AAdd(a, aEl[i]);	ENDIF
	NEXT
	RETURN a
...
//-------------------------------------------------------------------------------------------
METHOD NameFits( s, c ) CLASS oXML	// Check if search string s (wildcarded) fits cXmlName
	LOCAL i, j, r:=.T., a
	s := Upper(s)
	c := Upper(c)
	IF "*" $ s				// s contains wildcards
		a := hb_aTokens(s, "*")
		j := 1
		FOR i:=1 TO Len(a)
			IF !(r := (Empty(a[i]) .OR. (j := hb_At(a[i], c, j)) > 0));	EXIT;	ENDIF
			j += Len(a[i])
		NEXT
	ELSE					// s is just a simple name
		r := (Right(c, Len(s)) == s)	// Rightmost parts are equal
	ENDIF
	RETURN r
...
//	IF !IsNil(oParent) .AND. ValType(oParent) = "O";	::Parent := oParent;	ENDIF
	...
	IF !IsNil(cName) .AND. !(":" $ cName)
		...
		IF !(":" $ cName) .AND. !(Empty(sXmlNs) .AND. Empty(::aXmlNs))
			ErrGen("No namespace in " + cName)
			::OK := .F.
		ENDIF
	ENDIF
	::cXmlName := cName
...
//			CASE c == "V";		cMods += c	// Get oXml:Value, not oXML object
			CASE c $ ::cTypes;	cArea += c	// Area (EAU)
		...
		ENDCASE
	NEXT
	IF Right(cName,1) = "="					// xxx= - attribute: A_
		cArea := "A"
	...
	IF Empty(cArea);	cArea := "E";	ENDIF		// Area = Elements
